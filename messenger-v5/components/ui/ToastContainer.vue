  ...
  <Teleport to="body">
    <div class="tc">
      <TransitionGroup name="t">
        <div v-for="t in store.items" :key="t.id" :class="['toast','t-'+t.type]">
          <div class="t-dot"></div>
          <span>{{t.msg}}</span>
        </div>
      </TransitionGroup>
...
.toast{padding:12px 16px;border-radius:var(--r-lg);font-size:14px;font-weight:500;box-shadow:var(--sh-lg);display:flex;align-items:center;gap:10px;min-width:240px;max-width:360px;pointer-events:auto;background:#fff;border:1px solid var(--border)}
